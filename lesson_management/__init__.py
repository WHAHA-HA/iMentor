...
APP_NAME='lesson_mgmt'
lesson_mgmt = Blueprint(APP_NAME, __name__, url_prefix='/{0}'.format(APP_NAME),
    template_folder='templates')

@lesson_mgmt.app_errorhandler(403)
def handle_403(err):
    return render_template('{0}/403.html'.format(APP_NAME)), 403

@lesson_mgmt.app_errorhandler(404)
def handle_404(err):
    return render_template('{0}/404.html'.format(APP_NAME)), 404

@lesson_mgmt.app_errorhandler(500)
def handle_500(err):
