  ...
  grunt.initConfig({
    ...
    ngAnnotate: {
        ...
        app: {
            files: [
                {
                    expand: true,
                    cwd: '<%= pkg.approot %>static/lesson_mgmt/js',
    ...
    cssmin: {
      ...
      build: {
        files: [{
                expand: true,
                cwd: '<%= pkg.approot %>static/lesson_mgmt/css',
                src: ['./**/*.css','!./production/*'],
                dest: '<%= pkg.temp %>css',
                ext: '.css',
                extDot: 'last',       // Extensions in filenames begin after the last dot
      ...
      production: {
        files: [{
                '<%= pkg.approot %>static/lesson_mgmt/css/production/libs.min.css': [
                ...
                '<%= pkg.approot %>static/lesson_mgmt/css/production/app.min.css': [
                    '<%= pkg.temp %>css/header-bar.css',
                    '<%= pkg.temp %>css/lesson_mgmt.css'
                ]
            }
        ]
      }
    ...
    concat: {
        ...
        production:{
            files: {
                '<%= pkg.approot %>static/lesson_mgmt/js/production/libs.min.js': [
                    ...
                    '<%= pkg.libs %>angular-mask/mask.min.js',
                ...
                '<%= pkg.approot %>static/lesson_mgmt/js/production/app.min.js': [
    ...
    jshint: {
        dev: {
            src: [
                'gruntfile.js',
                '<%= pkg.approot %>static/lesson_mgmt/js/*.js', 
                '<%= pkg.approot %>static/lesson_mgmt/test/*.js',
                '!<%= pkg.approot %>static/lesson_mgmt/js/production/*'
            ],
            options: {
    ...
    watch: {
        ...
        scripts: {
            files: '<%= pkg.approot %>static/lesson_mgmt/js/**/*.js',
            tasks: ['jshint'],
            options: {
              spawn: false
            }
    ...
    copy: {
        build: {
            files: [
                ...
                // Get data
                {
                    cwd: '<%= pkg.approot %>static/lesson_mgmt/data/',
                    src: ['./**/*.*'],
                    dest: '<%= pkg.build %>data/',
                    expand: true,
                    flatten: false,
