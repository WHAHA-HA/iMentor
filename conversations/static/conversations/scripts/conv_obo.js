...
(function() {
    ...
    angular.module('irontec.simpleChat').directive('irontecSimpleChat', ['$timeout', SimpleChat]).filter('highlight', ['$sce',
        function($sce) {
    ...
    function SimpleChat($timeout) {
        var directive = {
            restrict: 'EA',
            templateUrl: '/new_platform/static/flipbook/nghtml/chatTemplate.html',
            replace: true,
            scope: {
                messages: '=',
                username: '=',
    ...
    function ChatCtrl($scope, $timeout, $rootScope, $http, $location, Upload) {
        ...
        var dateQueries = $location.search();
        var user = dateQueries['user'];
        var start_date = dateQueries['start'];
        var end_date = dateQueries['end']
        console.log(dateQueries['user']);
        console.log(dateQueries['start']);
        console.log(dateQueries['end']);

		$rootScope.conversationSnapshotStartDate = start_date;
		$rootScope.conversationSnapshotEndDate = end_date;

        $rootScope.openChatAdmin = function(user_id, start_date, end_date) {
            if(!angular.isDefined(user_id) || !angular.isDefined(start_date) || !angular.isDefined(end_date)){
                return;
            }
            console.log("user_id" + user_id, "start_date" + start_date, "end_date" + end_date);
            ...
            $scope.getMessagesDate = function() {
                if ($rootScope.user_id2) {
                    return $http.get('/conv/chat/date_range/' + start_date + '/' + end_date + '/' + $rootScope.user_id2 + '/', {});
                } else {
                    return $http.get('/conv/chat/date_range/', {});
                }
            };
            ...
            $scope.getUserInfo()
                .then(function(data) {
                	$rootScope.chatTitle = "Loading...";
                    $rootScope.useridnum2 = data.data.self.user_id;
                    $rootScope.firstname2 = data.data.self.f_name;
                    if (typeof data.data.mentor !== 'undefined' && typeof data.data.mentor.f_name !== 'undefined') {
                        ...
                        $rootScope.mentorId2 = data.data.mentor.user_id;

                        $rootScope.otherPersonMentee = undefined;
                    	$rootScope.otherMenteePhoto = undefined;
                        $rootScope.menteeId2 = undefined;


                        $rootScope.chatDefautTitle = "Message with " + $rootScope.otherPersonMentor;
                        console.log("$rootScope.mentorId " + $rootScope.mentorId2)
                        ...
                        $rootScope.menteeId2 = data.data.mentee.user_id;

                        $rootScope.otherPersonMentor = undefined;
                   		$rootScope.otherMentorPhoto = undefined;
                        $rootScope.mentorId2 = undefined;

                        $rootScope.chatDefautTitle = "Message with " + $rootScope.otherPersonMentee;
                        console.log("$rootScope.menteeId " + $rootScope.menteeId2)
                    }
                    return $scope.getMessagesDate();
                })
                .then(function(data) {
                    console.log(data);
                    if(vm.messages){
                    ...
                    if(vm.messages && vm.messages.length>=0 && data.data.messages.length>0) {
                    	
                    	Array.prototype.push.apply(vm.messages, data.data.messages);
                    }
        ...
        var vm = this;
        //vm.user = 'aaa';
        vm.firstname = $rootScope.firstname2;
        vm.isHidden = false;
        ...
        function submitFunction(message) {
            ...
            vm.sendingTxt = true;
            if (!message) { message = vm.writingMessage; }
		...
		if (user && start_date && end_date) {
            $rootScope.openChatAdmin(user, start_date, end_date);
        }
