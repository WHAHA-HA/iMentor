  ...
  grunt.initConfig({
    ...
    ngAnnotate: {
        ...
        app: {
            files: [
                {
                    expand: true,
                    cwd: '<%= pkg.approot %>static/menu/js',
    ...
    cssmin: {
      ...
      build: {
        files: [{
                expand: true,
                cwd: '<%= pkg.approot %>static/menu/css',
                src: ['./**/*.css','!./production/*'],
                dest: '<%= pkg.temp %>css',
                ext: '.css',
                extDot: 'last',       // Extensions in filenames begin after the last dot
      ...
      production: {
        files: [{
                '<%= pkg.approot %>static/menu/css/production/libs.min.css': [
                ...
                '<%= pkg.approot %>static/menu/css/production/app.min.css': [
                    '<%= pkg.temp %>css/menu.css',
    ...
    concat: {
        ...
        production:{
            files: {
                '<%= pkg.approot %>static/menu/js/production/libs.min.js': [
                ...
                '<%= pkg.approot %>static/menu/js/production/app.min.js': [
                    '<%= pkg.temp %>js/menu.js',
    ...
    jshint: {
        dev: {
            src: [
                'gruntfile.js',
                '<%= pkg.approot %>static/menu/js/*.js', 
                '<%= pkg.approot %>static/menu/test/*.js',
                '!<%= pkg.approot %>static/menu/js/production/*'
            ],
            options: {
    ...
    watch: {
        ...
        scripts: {
            files: '<%= pkg.approot %>static/menu/js/**/*.js',
            tasks: ['jshint'],
            options: {
              spawn: false
            }
    ...
    copy: {
        build: {
            files: [
                ...
                // Get data
                {
                    cwd: '<%= pkg.approot %>static/menu/data/',
                    src: ['./**/*.*'],
                    dest: '<%= pkg.build %>data/',
                    expand: true,
                    flatten: false,
