...
APP_NAME='mentor_rubric'
mentor_rubric = Blueprint(APP_NAME, __name__, url_prefix='/{0}'.format(APP_NAME),
    template_folder='templates')

@mentor_rubric.app_errorhandler(403)
def handle_403(err):
    return render_template('{0}/403.html'.format(APP_NAME)), 403

@mentor_rubric.app_errorhandler(404)
def handle_404(err):
    return render_template('{0}/404.html'.format(APP_NAME)), 404

@mentor_rubric.app_errorhandler(500)
def handle_500(err):
