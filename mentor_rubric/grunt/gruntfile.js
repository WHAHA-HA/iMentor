  ...
  grunt.initConfig({
    ...
    ngAnnotate: {
        ...
        app: {
            files: [
                {
                    expand: true,
                    cwd: '<%= pkg.approot %>static/mentor_rubric/js',
    ...
    cssmin: {
      ...
      build: {
        files: [{
                expand: true,
                cwd: '<%= pkg.approot %>static/mentor_rubric/css',
                src: ['./**/*.css','!./production/*'],
                dest: '<%= pkg.temp %>css',
                ext: '.css',
                extDot: 'last',       // Extensions in filenames begin after the last dot
      ...
      production: {
        files: [{
                '<%= pkg.approot %>static/mentor_rubric/css/production/libs.min.css': [
                ...
                '<%= pkg.approot %>static/mentor_rubric/css/production/app.min.css': [
                    '<%= pkg.temp %>css/header-bar.css',
                    '<%= pkg.temp %>css/mentor_rubric.css',
    ...
    concat: {
        ...
        production:{
            files: {
                '<%= pkg.approot %>static/mentor_rubric/js/production/libs.min.js': [
                ...
                '<%= pkg.approot %>static/mentor_rubric/js/production/app.min.js': [
                    '<%= pkg.temp %>js/app.js',
                    '<%= pkg.temp %>js/datacontext.service.js',
                    '<%= pkg.temp %>js/mentor_rubric.controller.js',
    ...
    jshint: {
        dev: {
            src: [
                'gruntfile.js',
                '<%= pkg.approot %>static/mentor_rubric/js/*.js', 
                '<%= pkg.approot %>static/mentor_rubric/test/*.js',
                '!<%= pkg.approot %>static/mentor_rubric/js/production/*'
            ],
            options: {
    ...
    watch: {
        ...
        scripts: {
            files: '<%= pkg.approot %>static/mentor_rubric/js/**/*.js',
            tasks: ['jshint'],
            options: {
              spawn: false
            }
    ...
    copy: {
        build: {
            files: [
                ...
                // Get data
                {
                    cwd: '<%= pkg.approot %>static/mentor_rubric/data/',
                    src: ['./**/*.*'],
                    dest: '<%= pkg.build %>data/',
                    expand: true,
                    flatten: false,
