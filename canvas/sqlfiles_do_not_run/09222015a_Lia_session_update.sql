...
SET sess_id = 36558
WHERE id IN (SELECT id FROM reporting_allmatchemail WHERE partner_id = 2150 AND send_date BETWEEN '2015-09-16 12:00:00' AND '2015-09-22 11:59:59');
--2 rows

/*
select id, name from partners_partner where name ilike 'Brashear 18-PD 3'
select * from partners_session where partner_id = 2150 order by start desc limit 10;
...
select * from canvas_set_copy where persona_id in (select persona_id from iuser_persona_partners where partner_id = 2150) order by parent_id asc;
select id, name from partners_partner where id = 2150;
*/