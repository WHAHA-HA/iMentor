...
WITH session_email_details AS (
	...
	WHERE ra.send_date::TIMESTAMPTZ AT TIME ZONE 'UTC' >= ps.start
		...
		AND ps.partner_id = 2310
		AND ps.start >= '10/13/15'
...
select * from partners_session where partner_id = 2310 order by start desc limit 15;
...
select id, name from partners_partner where name ilike '%JWHS ''18%';
select * from canvas_curriculum where id = 41
select id, name from partners_partner where id = 2310;
*/