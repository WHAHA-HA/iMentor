...
--2142, CHS'17LeadersinAction

begin transaction;

DELETE from reporting_matchcompletesession * WHERE session_id = 37275;
--27 rows. 27 affected.

DELETE from partners_session_curriculum * WHERE id = 935;
--1 row. 1 affected.

DELETE from partners_session * WHERE id = 37275;
--1 row. 1 affected.

/*
select id, name from partners_partner where name ilike 'chs%';
select * from partners_session where partner_id = 2142 order by start desc limit 15;
select * from partners_session_curriculum where p_session_id = 37275;
select * from reporting_matchcompletesession where session_id = 37275;
