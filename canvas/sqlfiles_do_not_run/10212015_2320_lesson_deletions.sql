...
where csc.parent_id in (85,86,87)
	and csc.persona_id in (select persona_id from iuser_persona_partners where partner_id = 2320)
...
--388 rows. 64 affected.

/*
select * from canvas_set_copy where id = 27096

select * from canvas_curriculum_copy where user_id = 169861 order by end_datetime asc
select * from canvas_set_copy where user_id = 169861 order by end_datetime asc
select id, email from iuser_user where id = 169861
