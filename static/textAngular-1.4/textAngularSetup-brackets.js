...
var textAngularSetupToolbar = ['bold', 'italics', 'underline', 'h5','h3','h1', 'insertLink', 'fontName'];
// Special edge cas efor IE or Edge browser
if(typeof detectIE === 'function') {
  var version = detectIE();
  if (version === false) {
    textAngularSetupToolbar.push('insertEmoji');
  }
} else {
  textAngularSetupToolbar.push('insertEmoji');
}
angular.module('textAngularSetup', ['ngSanitize', 'colorpicker.module', 'ui.bootstrap.dropdownToggle','imImageSelector','imEmbed', 'imUpload'])

// Here we set up the global display defaults, to set your own use a angular $provider#decorator.
.value('taOptions',  {
  toolbar: [
    textAngularSetupToolbar, //'italics',
  ],
  classes: {
    focussed: "focussed",
...
.run(['taRegisterTool', '$window', 'taTranslations', 'taSelection', 'taCustomRenderers','imImages','imEmbedService','imUploadService', '$sanitize', function(taRegisterTool, $window, taTranslations, taSelection, taCustomRenderers,imImages,imEmbedService, imUploadService, $sanitize){
  ...
  angular.forEach(['h1','h2','h3','h4','h5','h6'], function(h){
    taRegisterTool(h.toLowerCase(), {
      ...
      action: headerAction,
      class: 'fontsizeBtn btn btn-default',
  ...
  taRegisterTool('italics', {
    class: 'italicsBtn btn btn-default',
  ...
  taRegisterTool('underline', {
    class: 'underlineBtn btn btn-default',
  ...
  taRegisterTool('fontName', {
    tooltiptext: "Font",
    class: 'fontNameBtn btn btn-default',
    display: "<span class='bar-btn-dropdown dropdown' ng-disabled='isDisabled()'>" +
    "<button class='btn btn-default dropdown-toggle' type='button' tabindex='-1' ng-disabled='isDisabled()'><span ng-bind='active'> </span><span class='fa fa-font'></span></button>" +
    "<ul class='dropdown-menu' ng-disabled='isDisabled()'><li ng-repeat='o in options'><button ng-disabled='isDisabled()' class='checked-dropdown font-dropdown' type='button' ng-click='action(o.css)'><i ng-if='o.active' class='fa fa-check'></i> " +
    "<span  ng-bind='o.name' ng-if='o.css == \"Courier New\" '  style='font-family: \"Courier New\" '  ></span>" +
    "<span  ng-bind='o.name' ng-if='o.css == \"Georgia\" '  style='font-family: \"Georgia\" '  ></span>" +
    "<span  ng-bind='o.name' ng-if='o.css == \"Helvetica\" '  style='font-family: \"Helvetica\" '  ></span> " +
    "<span  ng-bind='o.name' ng-if='o.css == \"Verdana\" '  style='font-family: \"Verdana\" '  ></span> " +
    "</button></li></ul></span>",
  ...
  taRegisterTool('insertEmoji', {
    display: "<span class='insert-emoji  bar-btn-dropdown dropdown'>" +
    "<button class='btn btn-default dropdown-toggle' type='button' ng-disabled='showHtml()'>" +
    "<span> <i class='fa fa-smile-o'></i></span></button>" +
    "<ul class='dropdown-menu'><li ><div class='col-md-12'> " +
    "<tabset> <tab ng-repeat='category in categorisedEmojis' ng-click='toggleDropdown($event)' > <tab-heading> " +
    "<a  href class='emoji-menu-category'><img ng-src='[[getImageUrl(category.image)]]' alt='[[category.name]]'/></a> </tab-heading> " +
    "<div ng-repeat='emoji in category.emojis' class='emoji-menu'> <a href='' ng-click='action(emoji)' title='[[emoji.short_name]]' ng-mouseover='hover(emoji)'>" +
    "<img ng-src='[[getImageUrl(emoji.image)]]'/></a> </div> <div class='clearfix'></div></tab> </tabset> " +
    "<div class='clearfix'></div><div class='emoji-menu-footer'> <div class='col-md-3'> " +
    "<img ng-src='[[getImageUrl(currentEmoji.image)]]' > </div><div cclass='col-md-9'> " +
    "<span >{{currentEmoji.name}}</span> <br><span >:[[currentEmoji.short_name]]:</span> </div><div> " +
    "<span><i class='fa fa-info-circle'></i> &nbsp;Type <b>':'</b> and hit TAB key for autocomplete</span> </div></div><div></li></ul></span>",
