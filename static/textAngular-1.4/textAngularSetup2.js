...
.run(['taRegisterTool', '$window', 'taTranslations', 'taSelection', 'taCustomRenderers','imImages','imEmbedService','imUploadService', '$sanitize', function(taRegisterTool, $window, taTranslations, taSelection, taCustomRenderers,imImages,imEmbedService, imUploadService, $sanitize){
  ...
  taRegisterTool('fontName', {
    ...
    "<button class='btn btn-default dropdown-toggle' type='button' tabindex='-1' ng-disabled='showHtml()'><span>{{active}} </span><i class='fa fa-font'></i></button>" +
    "<ul class='dropdown-menu'><li ng-repeat='o in options'><button class='checked-dropdown font-dropdown' style='font-family: {{o.css}}' type='button' ng-click='action(o.css)'><i ng-if='o.active' class='fa fa-check'></i>{{o.name}}</button></li></ul></span>",
