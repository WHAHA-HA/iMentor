	...
	.controller('GridsterCtrl', ['gridsterConfig', '$timeout',
		function(gridsterConfig, $timeout) {
			...
			this.canItemOccupy = function(item, row, column) {
				return row > -1 && column > -1 && item.sizeX + column <= this.columns && item.getSizeY() + row <= this.maxRows;
			};
			...
			this.autoSetItemPosition = function(item) {
				...
				for (var rowIndex = 0; rowIndex < this.maxRows; ++rowIndex) {
					for (var colIndex = 0; colIndex < this.columns; ++colIndex) {
						...
						var items = this.getItems(rowIndex, colIndex, item.sizeX, item.getSizeY(), item);
			...
			this.getItem = function(row, column, excludeItems) {
				...
				while (row > -1) {
					...
					while (col > -1) {
						...
						if (items) {
							var item = items[col];
							if (item && (!excludeItems || excludeItems.indexOf(item) === -1) && item.sizeX >= sizeX && item.getSizeY() >= sizeY) {
								return item;
							}
						}
						++sizeX;
		...
		function($document, $timeout, $window, GridsterTouch) {
			function GridsterDraggable($el, scope, gridster, item, itemOptions) {
				...
				function drag(event) {
					...
					var itemsInTheWay = gridster.getItems(row, col, item.sizeX, item.getSizeY(), item);
					var hasItemsInTheWay = itemsInTheWay.length !== 0;

					if (gridster.swapping === true && hasItemsInTheWay) {
						var boundingBoxItem = gridster.getBoundingBox(itemsInTheWay),
							sameSize = boundingBoxItem.sizeX === item.sizeX && boundingBoxItem.getSizeY() === item.getSizeY(),
						...
						} else if (boundingBoxItem.sizeX <= item.sizeX && boundingBoxItem.getSizeY() <= item.getSizeY() && inline) {
				...
				function dragStop(event) {
					...
					var row = gridster.pixelsToRows(elmY);
					var col = gridster.pixelsToColumns(elmX);
					if (gridster.pushing !== false || gridster.getItems(row, col, item.sizeX, item.getSizeY(), item).length === 0) {
						item.row = row;
						item.col = col;
					}
		...
		function GridsterResizable($el, scope, gridster, item, itemOptions) {
			...
			function ResizeHandle(handleClass) {
				...
				function resize(e) {
					...
					var isChanged = item.row !== oldRow || item.col !== oldCol || item.sizeX !== oldSizeX || item.getSizeY() !== oldSizeY;
		...
		function($parse, GridsterDraggable, GridsterResizable, gridsterDebounce) {
			return {
				...
				link: function(scope, $el, attrs, controllers) {
					...
					scope.$watch(function() {
						return item.getSizeY() + ',' + item.sizeX + ',' + item.minSizeX + ',' + item.maxSizeX + ',' + item.minSizeY + ',' + item.maxSizeY;
					}, sizeChanged);
