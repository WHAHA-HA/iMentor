...
(function(angular){
    ...
    fsm.directive('fsmStickyHeader', [function(){
		...
		var originalLeft = $(element[0]).offset().left;
			...
			if(clonedColumns.length >0 ) {
		                header.find('th').each(function (index, column) {
		                    ...
		                    clonedColumn.outerWidth($(column).outerWidth()); //clonedColumn.css( 'width', column.offsetWidth + 'px'); //fixed thead width
		                    // fluid thead / table
				 ...
				 clonedColumns = clonedHeader.find('td');
				header.find('td').each(function (index, column) {
		                    var clonedColumn = $(clonedColumns[index]);
		                    clonedColumn.outerWidth($(column).outerWidth()); //clonedColumn.css( 'width', column.offsetWidth + 'px'); //fixed thead width
		                    // fluid thead / table
		                    //var finalWidthSet = column.offsetWidth / ($(window).innerWidth()-20)*100; // $(window) can be replace with a custom wrapper / container
			...
			if(scope.fsmStickyHeader != false)
			{
                        if (!clonedHeader){
                            createClone();
		...
		function scrollHandler(){
			if(clonedHeader)	{
				...
				$(clonedHeader).css('left', originalLeft - scrollLeft);
			}
		}
            }
        };
